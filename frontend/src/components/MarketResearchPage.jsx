  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 14,
        boxShadow: hovered ? '0 4px 16px rgba(245,197,24,0.15)' : '0 1px 3px rgba(0,0,0,0.06)',
...
function EmptyState({ onSubmit }) {
  return (
    <div className="fade-up" style={{ textAlign: 'center', paddingTop: 40 }}>
      ...
      <p style={{ fontSize: 14, color: '#4a5568', maxWidth: 440, margin: '0 auto 36px', lineHeight: 1.7 }}>
      ...
      <div className="grid-3 example-cards" style={{ maxWidth: 720, margin: '0 auto' }}>
        {EXAMPLE_CARDS.map(card => (
          <ExampleCard key={card.title} {...card} onClick={() => onSubmit(card.title)} />
        ))}
      </div>
    </div>
  )
...
export default function MarketResearchPage({
  phase, currentQuery, statusLabel, jobData, errorInfo,
  onSubmit, onCancel, onNewSearch,
}) {
  ...
  return (
    <div style={{ minHeight: '100%', background: '#f0f2f5', fontFamily: 'Inter, sans-serif' }}>

      {/* Top action bar */}
      <div className="topbar-pad" style={{ display: 'flex', justifyContent: 'flex-end', padding: '16px 28px 0' }}>
        {showResult ? (
          <div style={{ display: 'flex', gap: 10 }}>
            <ExportButton query={currentQuery} jobData={jobData} />
            <button onClick={onNewSearch} style={{
              padding: '8px 16px', borderRadius: 8, background: '#e63946', color: '#fff',
      ...
      <div className="page-pad" style={{ padding: '16px 28px 0' }}>
        <HeroSearch onSubmit={onSubmit} defaultQuery={currentQuery} disabled={showLoading} />
      </div>

      {/* Page body */}
      <div className="body-pad" style={{ padding: '24px 28px 60px' }}>
        {showLoading && <AnalyzingPanel query={currentQuery} status={statusLabel} onCancel={onCancel} />}
        {showResult  && <ResultsPanel jobData={jobData} query={currentQuery} onNewSearch={onNewSearch} />}
        {showError   && <ErrorPanel error={errorInfo} onRetry={onNewSearch} />}
        {showEmpty   && <EmptyState onSubmit={onSubmit} />}
      </div>
    </div>
  )
}
